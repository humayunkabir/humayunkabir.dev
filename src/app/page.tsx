...
const SocialIcon = ({ href, name }: SocialIconProps) => (
	<a
		...
		target="_blank"
		rel="noreferrer"
		className="group relative flex items-center justify-center w-12 h-12 rounded-full bg-zinc-900/80 border border-zinc-800 backdrop-blur-sm transition-all duration-300 hover:scale-110 hover:bg-zinc-800 hover:border-zinc-700"
	>
		<Icon
			name={name}
			className="w-5 h-5 text-zinc-400 group-hover:text-zinc-100 transition-colors duration-300"
		/>
	</a>
);
...
export default function Home() {
	return (
		<main className="flex-1 flex flex-col items-center justify-center w-full px-6 py-20 overflow-hidden relative">
			...
			<div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-emerald-500/10 rounded-full blur-[120px] pointer-events-none opacity-50" />
			<div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-purple-500/10 rounded-full blur-[100px] pointer-events-none opacity-40" />

			<div className="w-full max-w-3xl mx-auto space-y-16 opacity-0 animate-[fade-in_1s_ease-out_forwards] z-10 relative">
				<header className="space-y-6 text-center sm:text-left">
					...
					<h1 className="text-4xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-zinc-100 to-zinc-500 sm:text-6xl pb-2">
